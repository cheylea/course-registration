DROP PROCEDURE IF EXISTS SP_TeacherAddMarks;
	...
	IF RoleCheck_Teacher_Var IS NULL OR RoleCheck_Teacher_Var <> 2 
		...
		ELSE IF
			...
			THEN
				...
				SELECT 3
				INTO result;
			ELSE IF EnrolmentCheck_Var IS NULL
				THEN
					# Enrolment does not exist
					SELECT 4
					INTO result;
				ELSE IF Provided_Mark <> 'Pass' AND Provided_Mark <> 'Fail'
					THEN
						# Invalid grade provided
						SELECT 5
						INTO Result;
				ELSE IF Provided_Mark = 'Pass'
					THEN
						#Update mark to pass
